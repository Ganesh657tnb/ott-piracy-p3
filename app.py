...
# --- 1. SETUP & CONFIGURATION (Definition First!) ---
DB_NAME = "guardian.db"
UPLOAD_DIR = "master_videos"  # Defined here first
...
# --- 2. AES-128 & DSSS LOGIC ---

def encrypt_user_id(user_id):
    """Encrypts only the User ID using AES-128."""
...
def decrypt_user_id(bit_string):
    """Decrypts bits back to User ID using AES-128."""
    try:
        key = SECRET_KEY.ljust(16)[:16].encode()
...
# --- 3. MAIN APP ---
def main():
    st.set_page_config(page_title="Guardian Anti-Piracy v2.7", layout="wide")
    init_db()
    ...
    if st.session_state.uid is None:
        ...
        with c2:
            ...
            nu, npw = st.text_input("Name"), st.text_input("Pass", type="password")
            if st.button("Sign Up"):
                h = bcrypt.hashpw(npw.encode(), bcrypt.gensalt())
                conn = sqlite3.connect(DB_NAME)
                try:
                    conn.execute("INSERT INTO users (username, password) VALUES (?,?)", (nu, h))
                    conn.commit()
                    st.success("User created!")
                except: st.error("Error creating user.")
        st.stop()

    st.sidebar.title("Guardian v2.7")
    if st.sidebar.button("Logout"):
        st.session_state.uid = None
        st.query_params.clear()
        st.rerun()

    t1, t2, t3 = st.tabs(["📚 Shared Library", "📤 Upload", "🔍 Detector"])
    ...
    with t3:
        leak = st.file_uploader("Pirated File")
        if leak and st.button("Identify Pirater"):
            with tempfile.TemporaryDirectory() as tmp:
                ...
                if res: st.error(f"PIRATER ID: {res}"); st.balloons()
